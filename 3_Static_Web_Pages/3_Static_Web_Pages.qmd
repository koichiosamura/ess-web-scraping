...
poster_titles <- html |> 
  html_elements(".wrapper.style3:contains(posters)") |> 
  html_elements("b") 
length(poster_titles)

poster_authors <- html |> 
  html_elements(".wrapper.style3:contains(posters)") |> 
  html_elements("i")
length(poster_authors)


#| class: fragment
poster_data <- map2(titles_poster, sessions_poster, function(x, y) {
  chair <- x |> 
    html_element("i") |> 
    html_text()
...
```{r}
html <- readLines("https://ecpr.eu/Events/AcademicProgramme/Schedule?eventID=224")
object.size(html) |> 
  format(units = "MB")
```
...
```{r}
html <- readLines("https://ecpr.eu/Events/AcademicProgramme/Schedule?eventID=224", encoding = "windows-1252") |>
  paste(collapse = "\n") |>
  read_html()
...
```{r}
#| eval: false
check_in_browser(ecpr_data_df$Papers[5])
```
...
```{r}
html <- read_html(ecpr_data_df$Papers[5])
paper_title <- html |>
  html_elements("[href*='PaperDetails']") |>
  html_text2()
paper_title
...
```{r}
#| class: fragment
extract_papers(ecpr_data_df$Papers[5])
```
...
```{r}
pdf_exercise <- pdf_data("/Users/Koichi/ucloud/Summer School/Webscraping/Untitled/3_Static_Web_Pages/data/example.pdf", font_info = TRUE)
pdf_exercise[[1]]
```
...
tidy_page <- function(page) {
  left_column <- page |> 
    ...
    group_by(y) |> 
    summarise(text = paste(text, collapse = " "),
              font_size = list(unique(font_size)))
...
```{r}
pdf_exercise_data <- pdf_exercise[1:3] |> 
  # bring the text in the correct order on each page
  map(tidy_page) |> 
  bind_rows() |> 
  # paper titles use a specific font, which make them easy to identify
  mutate(section_title = font_size == 20) |> 
  # lines with more than one font (bold and non-bold) are meta information, let's note that
  #mutate(font_name = map_chr(font_name, function(x) ifelse(length(x) > 1, "meta", x))) |> 
  # Panels start with the string panel and a large bold font
  mutate(text_section = font_size == 11) |> 
  # using cumsum, we count the TRUE values. One appears at the start of each panel
  #mutate(panel = cumsum(panel_start)) |> 
  # we are not interested in the information before the first panel
  filter(panel > 0) |> 
 ...
 # mutate(paper_nr = cumsum(paper_title & !lag(paper_title))) |> 
  ...
  summarise(
    # we select all text where the paper_nr == 0, which is te panel title
    section_title = paste(text[paper_nr == 0], collapse = " "),
    # now we combine the text in the rows that have a paper title
    text_section = paste(text[paper_title], collapse = " "),
    # finally, we put the authors in a list. They are the text within a panel descript that uses one of two fonts
    #author = list(text[font_name %in% c("ZXOBMV+SFRM1000", "ZGRRLU+SFBX1000")]),
    .groups = "drop"
  ) |> 
